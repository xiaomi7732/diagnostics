...
namespace APMExp.Models.TraceSessions
{
    public sealed class MonitorTraceSession : TraceSessionId, IDisposable
    {
        ...
        public event Action<TraceSessionMetric> Report;
        ...
        public void OnReport(string metricName, double metricValue)
        {
            ...
            Report?.Invoke(new TraceSessionMetric()
            {
                ProcessId = ProcessId,
                SessionId = Id,
        ...
        private void Dynamic_All(TraceEvent obj)
        {
            ...
            if (obj.EventName.Equals("EventCounters"))
            {
                ...
                OnReport(payload.GetDisplay() ?? payload.GetName(), payload.GetValue());
            }
        }
    }
}