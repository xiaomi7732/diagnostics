...
{
    public sealed class KnownCounterProvider
    {
        private readonly IReadOnlyDictionary<string, CounterProvider> _knownProviders;
        
        # region Singleton
        private static KnownCounterProvider _instance = new KnownCounterProvider();
        private KnownCounterProvider()
        {
            _knownProviders = CreateKnownProviders().ToDictionary(p => p.Name, StringComparer.OrdinalIgnoreCase);
        }
        public static KnownCounterProvider Instance { get; } = _instance;
        #endregion

        public IReadOnlyList<CounterProvider> GetAllProviders() => _knownProviders.Values.ToList();

        public bool TryGetProvider(string providerName, out CounterProvider provider) => _knownProviders.TryGetValue(providerName, out provider);
        private IEnumerable<CounterProvider> CreateKnownProviders()
        {
